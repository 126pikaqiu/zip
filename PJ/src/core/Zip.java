    ...
    private void compress(LZipOutputStream zos, File sourceFile, String base) throws Exception
    {
        ...
        else//如果不是目录（文件夹），即为文件，则先写入目录进入点，之后将文件写入zip文件中
        {
            if(sourceFile.length() == 0){ //空文件
                zos.putNextEntry(  base, 2);//2表示空文件
            }else{
...
//                multiThreadScan(zos,fis1);
    ...
    private void multiThreadScan(LZipOutputStream out, FileInputStream fileInputStream) throws FileNotFoundException, InterruptedException {//用三个线程同时扫描
        Thread t1 = new Thread(new MultiThreadScan(fileInputStream,out),"scan1");
        Thread t2 = new Thread(new MultiThreadScan(),"scan2");
        Thread t3 = new Thread(new MultiThreadScan(),"scan3");
        t1.start();
        t2.start();
        t3.start();
